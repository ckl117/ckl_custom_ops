...
__global__ void __launch_bounds__(256)
    hgemm_mma_m16n8k16_mma2x4_warp4x4_kernel(T *A, T *B, T *C, int M, int N,
                                             int K) {
  ...
  __shared__ T s_a[BM][BK + A_PAD]; // 128*16*2=4KB
  __shared__ T s_b[BK][BN + B_PAD]; // 16*128*2=4KB, 16*(128+16)*2=4.5KB

  const int tid = threadIdx.y * blockDim.x + threadIdx.x; // within block

  // warp_0 warp_1 warp_2 warp_3 warp_4 warp_5 warp_6 warp_7
  // 二维转换 warp排布 列主序
  // warp_0 warp_2 warp_4 warp_6
  // warp_1 warp_3 warp_5 warp_7
...
#pragma unroll
    // warp_0 * WARP_TILE_M
    // --------------------------------------------------------
    // | warp_0 warp_0 warp_0 warp_0 | warp_2 ... | ... | ... |
    // | warp_0    ....              |    ...     | ... | ... |
    // | warp_0    ....              |    ...     | ... | ... |
    // | warp_0    ....              |    ...     | ... | ... |
    // | warp_1    ....              | warp_3 ... | ... | ... |
...
void hgemm_mma_m16n8k16_mma2x4_warp4x4(torch::Tensor a, torch::Tensor b,
  ...
  DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FP16(
      a.scalar_type(), scalar_t, ([&] {
        hgemm_mma_m16n8k16_mma2x4_warp4x4_kernel<
            scalar_t, MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M,
            WARP_TILE_N, A_PAD, B_PAD><<<grid, block>>>(
            reinterpret_cast<scalar_t *>(a.data_ptr()),
            reinterpret_cast<scalar_t *>(b.data_ptr()),
            reinterpret_cast<scalar_t *>(c.data_ptr()), M, N, K);
