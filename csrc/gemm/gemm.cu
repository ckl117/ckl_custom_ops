...
__global__ void gemm_simple_kernel(T *Cptr, const T *Aptr, const T *Bptr,
                                   const T *Biasptr, int m, int n, int k) {

  using namespace cute;

  // Tensor A = make_tensor(make_gmem_ptr(Aptr), make_shape(m, k),
  // make_stride(k, Int<1>{})); Tensor B = make_tensor(make_gmem_ptr(Bptr),
  // make_shape(n, k), make_stride(k, Int<1>{})); Tensor C =
  // make_tensor(make_gmem_ptr(Cptr), make_shape(m, n), make_stride(n,
  // Int<1>{}));
  ...
  // make_coord(iy, _)); Tensor gB = local_tile(B, make_tile(Int<kTileN>{},
  // Int<kTileK>{}), make_coord(ix, _)); Tensor gC = local_tile(C,
  // make_tile(Int<kTileM>{}, Int<kTileN>{}), make_coord(iy, ix));
...
void gemm_simple(torch::Tensor &d, torch::Tensor &a, torch::Tensor &b) {
  ...
  //     TORCH_CHECK(bias->dtype() == d.dtype(), "bias dtype must match output
  //     dtype");
  // }
  ...
  gemm_simple_kernel<half, kTileM, kTileN, kTileK, MMA><<<blocks, threads, 0>>>(
      reinterpret_cast<half *>(d.data_ptr()),
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b.data_ptr()), nullptr, m, n, k);
  // DISPATCH_PYTORCH_DTYPE_TO_CTYPE_FLOAT_FP16(a.scalar_type(), scalar_t, ([&]
  // {
  //     gemm_simple_kernel<scalar_t, kTileM, kTileN, kTileK, MMA><<<blocks,
  //     threads, 0, stream>>>(
